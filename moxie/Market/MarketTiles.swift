...
class MarketTiles: UITableViewCell {
    ...
    let mainImageView: UIImageView = {
        ...
        t.translatesAutoresizingMaskIntoConstraints = false
        t.contentMode = .scaleAspectFit
        t.image = #imageLiteral(resourceName: "marketLoadingPlace")
    ...
    override func layoutSubviews() {
        ...
        if let panelImage = mainImage {
            mainImageView.image = panelImage
        }
        
        if let panelButton = actionButtonTitle {
            //actionButton.setTitle(panelButton, for: .normal)
            
            //actionButton.titleLabel?.font = UIFont.init(name: "AvenirNext-Medium", size: 15)
